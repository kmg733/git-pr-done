...
> 6번에 대한 단서: 이 단계는 **동의를 묻기 전에** 서버와 통신한다. 프롬프트에서 아니오를 골라도 내 브랜치와 파일은 그대로지만, 서버 브랜치 목록의 로컬 사본은 이미 갱신된 뒤다.
...
`--force`는 **로컬 이력 검사만 건너뛴다.** 머지됐다는 증거(git 이력 또는 서버 조회) 자체는 여전히 요구한다. 증거가 없으면 거부하고 무엇을 잃게 되는지 보여준다.
...
| `머지 증거가 없어 강제 삭제를 거부합니다` | `--force`로도 머지 증거를 찾지 못했다 | 나열된 커밋은 다른 어디에도 없다. 확신할 때만 `git branch -D <브랜치>` |
